...
fix-abci-app-specs:
	...
	autonomy analyse fsm-specs --update --app-class VeraAbciApp --package packages/dvilela/skills/vera_abci/ || (echo "Failed to check vera_abci abci consistency" && exit 1)
	autonomy analyse fsm-specs --update --app-class VeraChainedSkillAbciApp --package packages/dvilela/skills/vera_chained_abci/ || (echo "Failed to check vera_chained_abci abci consistency" && exit 1)
